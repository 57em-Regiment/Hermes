...
type useOptimisticIncrementStockMutationProps = {
  item: StockDetails;
};
export const useOptimisticIncrementStockMutation = ({
  item,
}: useOptimisticIncrementStockMutationProps) => {
  const queryClient = useQueryClient();
  const queryKey = InventoryFactory.StockInInventory(item.inventoryId);

  return useMutation({
    mutationFn: async (formValues: UpdateStock) =>
      await stockApi.increment({
        params: {
